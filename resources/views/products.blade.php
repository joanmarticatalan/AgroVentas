    ...
    <main class="flex-1">
        <section class="max-w-7xl mx-auto px-6 py-10 lg:py-14">
            <div class="grid gap-8 lg:grid-cols-[1.15fr_0.85fr] lg:items-end">
                <div class="space-y-4">
                    <span class="inline-flex items-center rounded-full bg-white/80 px-4 py-1 text-sm font-semibold text-agro-brown shadow-sm ring-1 ring-agro-brown/10">
                        Mercado agrícola sin rodeos
                    </span>
                    <div class="space-y-3">
                        <h1 class="text-4xl font-black tracking-tight text-agro-primary sm:text-5xl">
                            Encuentra producto fresco con filtros claros y una URL compartible.
                        </h1>
                        <p class="max-w-2xl text-lg leading-8 text-slate-700">
                            Acota por origen, precio, disponibilidad y fecha de producción sin salir del renderizado servidor.
                        </p>
                    </div>
                </div>

                <div class="rounded-[2rem] border border-agro-primary/10 bg-white/80 p-6 shadow-xl shadow-agro-primary/10 backdrop-blur">
                    <div class="flex items-center justify-between gap-4">
                        <div>
                            <p class="text-sm font-semibold uppercase tracking-[0.2em] text-agro-brown">Resultados</p>
                            <p class="mt-2 text-3xl font-black text-agro-primary">{{ $products->count() }}</p>
                        </div>
                        <div class="text-right text-sm text-slate-600">
                            @if($hasActiveFilters)
                                <p class="font-semibold text-agro-brown">Listado filtrado</p>
                                <p>Tu selección está activa</p>
                            @else
                                <p class="font-semibold text-agro-brown">Listado general</p>
                                <p>Explora todo el catálogo</p>
                            @endif
                        </div>
                    </div>
                </div>
            ...
            <section class="mt-10 rounded-[2rem] bg-white shadow-xl shadow-agro-primary/10 ring-1 ring-agro-primary/10 overflow-hidden">
                <div class="border-b border-slate-200 bg-gradient-to-r from-agro-primary to-agro-secondary px-6 py-6 text-white">
                    <div class="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
                        <div>
                            <h2 class="text-2xl font-bold">Filtrar productos</h2>
                            <p class="text-sm text-green-100">Ajusta la búsqueda y comparte la URL con el mismo resultado.</p>
                        </div>
                ...
                <form method="GET" action="{{ route('todos.productos') }}" class="p-6 lg:p-8">
                    <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
                        <div class="md:col-span-2 xl:col-span-2">
                            <label for="q" class="mb-2 block text-sm font-semibold text-slate-700">Buscar por nombre o variedad</label>
                            <input
                                id="q"
                                name="q"
                                type="text"
                        ...
                        <div>
                            <label for="disponibilidad" class="mb-2 block text-sm font-semibold text-slate-700">Disponibilidad</label>
                            <select
                                id="disponibilidad"
                                name="disponibilidad"
                                class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-base text-slate-900 outline-none transition focus:border-agro-primary focus:bg-white"
                        ...
                        <div>
                            <label for="precio_min" class="mb-2 block text-sm font-semibold text-slate-700">Precio mínimo</label>
                            <input
                                id="precio_min"
                                name="precio_min"
                                type="number"
                        ...
                        <div>
                            <label for="precio_max" class="mb-2 block text-sm font-semibold text-slate-700">Precio máximo</label>
                            <input
                                id="precio_max"
                                name="precio_max"
                                type="number"
                        ...
                        <div>
                            <label for="fecha_desde" class="mb-2 block text-sm font-semibold text-slate-700">Producción desde</label>
                            <input
                                id="fecha_desde"
                                name="fecha_desde"
                                type="date"
                        ...
                        <div>
                            <label for="fecha_hasta" class="mb-2 block text-sm font-semibold text-slate-700">Producción hasta</label>
                            <input
                                id="fecha_hasta"
                                name="fecha_hasta"
                                type="date"
                        ...
                        <div>
                            <label for="sort" class="mb-2 block text-sm font-semibold text-slate-700">Ordenar por</label>
                            <select
                                id="sort"
                                name="sort"
                                class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-base text-slate-900 outline-none transition focus:border-agro-primary focus:bg-white"
